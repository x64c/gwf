...
// on the Client for this session-data type (via bearer.SetUserlessRefreshSideloader).
...
func (sd *UserlessSessionData) UpstreamRequestWithBearerRetriable(
	ctx context.Context,
	...
	if !ok {
		return nil, http.StatusInternalServerError, errs.UpstreamRefreshSideloaderNotSet.WithDetail("Client " + fwClient.ID + " has no UserlessRefreshSideloader")
	}
...
func (sd *UserlessSessionData) UpstreamRequestWithBearer(
	ctx context.Context,
	fwClient *fwupstream.Client,
	method, endpoint string,
	payload *fwupstream.RequestPayload,
