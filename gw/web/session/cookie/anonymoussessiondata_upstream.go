...
func (sd *AnonymousSessionData) HoldUpstreamRefresh(ctx context.Context, fn func(ctx context.Context) error) error {
	return sd.Mgr.lockingManager.AcquireDoRelease(ctx, sd.Mgr.AnonymousSessionRowKey(sd.ID), fn)
}
...
// SetAnonymousRefreshSideloader; without one, returns
