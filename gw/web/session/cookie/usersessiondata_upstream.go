...
func (sd *UserSessionData[UID]) UpstreamAccessToken(ctx context.Context, clientID string) (string, *errs.Error) {
	...
	s.Once.Do(func() {
		s.Val, s.Err = sd.Mgr.UserFetchUpstreamAccessToken(ctx, sd.ID, clientID)
	})
	return s.Val, s.Err
}
...
func (sd *UserSessionData[UID]) UpstreamRefreshToken(ctx context.Context, clientID string) (string, *errs.Error) {
	...
	s := slot.(*fwupstream.TknSlot)
	s.Once.Do(func() {
		s.Val, s.Err = sd.Mgr.UserFetchUpstreamRefreshToken(ctx, sd.ID, clientID)
	})
	return s.Val, s.Err
}
...
// on the Client for this session-data type (via cookie.SetUserRefreshSideloader).
...
func (sd *UserSessionData[UID]) UpstreamRequestWithBearerRetriable(
	...
	if resErr := sd.Mgr.UserStoreUpstreamTokenPair(ctx, sd.ID, fwClient.ID, newPair.AccessToken, newPair.RefreshToken); resErr != nil {
